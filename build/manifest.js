export const manifest = {
	appDir: "_app",
	assets: new Set(["BingSiteAuth.xml","apexlinks-144.png","apexlinks-192.png","apexlinks-512.png","apple-touch-icon-1024x1024.png","apple-touch-icon-120x120.png","apple-touch-icon-152x152.png","apple-touch-icon-167x167.png","apple-touch-icon-180x180.png","favicon.ico","junegull.ttf","manifest.webmanifest","placeholder.png","service-worker.js"]),
	mimeTypes: {".xml":"application/xml",".png":"image/png",".ico":"image/vnd.microsoft.icon",".ttf":"font/ttf",".webmanifest":"application/manifest+json"},
	_: {
		entry: {"file":"start-5cb1499b.js","js":["start-5cb1499b.js","chunks/index-b9b9b215.js","chunks/singletons-d1fb5791.js"],"css":[]},
		nodes: [
			() => import('./server/nodes/0.js'),
			() => import('./server/nodes/1.js'),
			() => import('./server/nodes/3.js'),
			() => import('./server/nodes/2.js'),
			() => import('./server/nodes/4.js'),
			() => import('./server/nodes/7.js'),
			() => import('./server/nodes/6.js'),
			() => import('./server/nodes/8.js'),
			() => import('./server/nodes/5.js'),
			() => import('./server/nodes/9.js')
		],
		routes: [
			{
				type: 'page',
				id: "",
				pattern: /^\/$/,
				names: [],
				types: [],
				path: "/",
				shadow: null,
				a: [0,2],
				b: [1]
			},
			{
				type: 'endpoint',
				id: "send",
				pattern: /^\/send\/?$/,
				names: [],
				types: [],
				load: () => import('./server/entries/endpoints/send.js')
			},
			{
				type: 'endpoint',
				id: "robots.txt",
				pattern: /^\/robots\.txt$/,
				names: [],
				types: [],
				load: () => import('./server/entries/endpoints/robots.txt.js')
			},
			{
				type: 'page',
				id: "edit",
				pattern: /^\/edit\/?$/,
				names: [],
				types: [],
				path: "/edit",
				shadow: null,
				a: [0,3],
				b: [1]
			},
			{
				type: 'page',
				id: "login",
				pattern: /^\/login\/?$/,
				names: [],
				types: [],
				path: "/login",
				shadow: null,
				a: [0,4],
				b: [1]
			},
			{
				type: 'page',
				id: "t",
				pattern: /^\/t\/?$/,
				names: [],
				types: [],
				path: "/t",
				shadow: null,
				a: [0,5],
				b: [1]
			},
			{
				type: 'endpoint',
				id: "auth/login",
				pattern: /^\/auth\/login\/?$/,
				names: [],
				types: [],
				load: () => import('./server/entries/endpoints/auth/login.js')
			},
			{
				type: 'endpoint',
				id: "auth/join",
				pattern: /^\/auth\/join\/?$/,
				names: [],
				types: [],
				load: () => import('./server/entries/endpoints/auth/join.js')
			},
			{
				type: 'endpoint',
				id: "auth/exit",
				pattern: /^\/auth\/exit\/?$/,
				names: [],
				types: [],
				load: () => import('./server/entries/endpoints/auth/exit.js')
			},
			{
				type: 'page',
				id: "t/[id]",
				pattern: /^\/t\/([^/]+?)\/?$/,
				names: ["id"],
				types: [null],
				path: null,
				shadow: null,
				a: [0,6],
				b: [1]
			},
			{
				type: 'page',
				id: "u/[id]",
				pattern: /^\/u\/([^/]+?)\/?$/,
				names: ["id"],
				types: [null],
				path: null,
				shadow: null,
				a: [0,7],
				b: [1]
			},
			{
				type: 'page',
				id: "t/[id]/edit",
				pattern: /^\/t\/([^/]+?)\/edit\/?$/,
				names: ["id"],
				types: [null],
				path: null,
				shadow: null,
				a: [0,8],
				b: [1]
			},
			{
				type: 'page',
				id: "u/[id]/txt",
				pattern: /^\/u\/([^/]+?)\/txt\/?$/,
				names: ["id"],
				types: [null],
				path: null,
				shadow: null,
				a: [0,9],
				b: [1]
			}
		],
		matchers: async () => {
			
			return {  };
		}
	}
};
