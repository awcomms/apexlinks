...
if ("X_CUSTOM_") {
	for (const name in process.env) {
		if (name.startsWith("X_CUSTOM_")) {
			const unprefixed = name.slice("X_CUSTOM_".length);
			if (!expected.has(unprefixed)) {
				throw new Error(
					`You should change envPrefix (${"X_CUSTOM_"}) to avoid conflicts with existing environment variables — unexpectedly saw ${name}`
				);
			}
		}
	}
			...
			getClientAddress: () => {
				if (address_header) {
					...
					if (address_header === 'x-forwarded-for') {
						...
						if (xff_depth < 1) {
							throw new Error(`${"X_CUSTOM_" + 'XFF_DEPTH'} must be a positive integer`);
						}

						if (xff_depth > addresses.length) {
							throw new Error(
								`${"X_CUSTOM_" + 'XFF_DEPTH'} is ${xff_depth}, but only found ${
									addresses.length
								} addresses`
							);
						}
